...
averageJacobian=[];
for z=1:40
inputFID=fopen(['results_PE' suffix num2str(z) '.txt']);
line=fgetl(inputFID);
simFluxes=[];
fluxes=0;
while(line~=-1)
...
if(z==1)
    ...
    averageJacobian=averageJacobian+a2;
end
end
averageJacobian=averageJacobian/40;

%HeatMap(averageJacobian,'ColumnLabels',reactionLabels1(basisIdxs),'RowLabels',heatMapLabels)